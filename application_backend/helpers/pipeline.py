...
def run_solana_update_pipeline(forecast_days=None, force=False): # function to orchestrate one SOL data + prediction update cycle
    ...
    if not force and not needs_update(last_time): # OHLCV calendar-current
        bundle = tiger_db.read_market_bundle(forecast_days=forecast_days) # reload from Tiger
        pred_series = (bundle.get("predictions") or {}).get("series") or [] # forecast list
        if pred_series: # already have forward predictions — nothing to do
            logger.info("Tiger OHLCV + predictions already current - skipping fetch/predict") # skip work
        ...
        # Real prices current but forecasts missing (e.g. predictions table wiped) — fall through
        logger.info("Tiger OHLCV current but predictions empty - running predictor") # recover
        ohlcv_data = bundle.get("real") or tiger_db.read_ohlcv() # reuse bundle real series
        summary["updated"] = False # no yfinance fetch this cycle
        summary["rows_fetched"] = 0 # none
        summary["rows_upserted"] = 0 # none
        real_payload = frontend_delivery.build_real_only_payload(ohlcv_data) # predictions_pending
        frontend_delivery.send_to_frontend(real_payload) # show real while GPU runs
        ...
        summary["reason"] = "predictions_refilled" # OHLCV skipped, preds regenerated
